...
export default function MyLeases() {
  ...
  const [leaseResponse, setLeaseResponse] = useState(null);
  ...
  let requestbody = {};
  // const tenantId = localStorage.getItem('userId') **Need to Replace HardCoded Tenenat Id 
  useEffect(() => {
    instance.post(request.lease_search + "/" + "15a56797-2da9-4650-a92c-edf06c512484", requestbody, config)
      .then(response => {
        const leases = response.data;
        if (typeof leases !== 'undefined' && leases !== null && leases !== '' && leases.isSuccess === true && leases.result.length > 0) {
          let data = leases.result;
          setLeaseResponse(data);

          localStorage.setItem("Leases", JSON.stringify(data));
        } else {
  ...
  //   let unitNumbers = [];
  //   leaseResponse.forEach((element) => {
  //     console.log(element.unitInfo.unitNumber);
  //     unitNumbers.push(element.unitInfo.unitNumber)
